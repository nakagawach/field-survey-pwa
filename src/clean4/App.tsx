...
const createProject = (): SurveyProject => {
  ...
  return {
    id: crypto.randomUUID(), title: '', location: '', lotNumber: '', surveyDate: createdAt.slice(0, 10),
    landCategory: '', boundaryChecked: false, memo: '', createdAt, updatedAt: createdAt,
  }
}
...
const createPoint = (projectId: string, number: number): BoundaryPoint => {
  const createdAt = isoNow()
  return {
    id: crypto.randomUUID(), projectId, name: `P${number}`, markerType: '', condition: '', positionMemo: '', memo: '', createdAt, updatedAt: createdAt,
  }
}
...
function PhotoThumb({ photo, onOpen }: { photo: BoundaryPhoto; onOpen: () => void }) {
  ...
  return <button type="button" className="photo-thumb" onClick={onOpen} aria-label={`${photo.fileName}を全画面表示`}>{url && <img src={url} alt={photo.fileName} />}<span>{photo.category || '未分類'}</span></button>
}
...
export default function App() {
  ...
  const refreshProjects = async () => {
    ...
    const entries = await Promise.all(nextProjects.map(async (item) => {
      const nextPoints = await getBoundaryPointsByProjectId(item.id)
      const counts: Record<string, number> = {}
      await Promise.all(nextPoints.map(async (nextPoint) => { counts[nextPoint.id] = (await getPhotosByBoundaryPointId(nextPoint.id)).length }))
  ...
  const refreshProjectData = async (projectId: string) => {
    ...
    const counts: Record<string, number> = {}
    await Promise.all(nextPoints.map(async (item) => { counts[item.id] = (await getPhotosByBoundaryPointId(item.id)).length }))
  ...
  const openProject = async (nextProject: SurveyProject) => { setProject(nextProject); await refreshProjectData(nextProject.id); setScreen('projectDetail') }
  ...
  const removeProject = async (target: SurveyProject) => {
    ...
    await deleteProject(target.id); await refreshProjects()
  }

  const openPoint = async (nextPoint: BoundaryPoint) => { setPoint({ ...nextPoint, positionMemo: nextPoint.positionMemo ?? '' }); await refreshPhotos(nextPoint.id); setScreen('pointDetail') }
  ...
  const savePointDraft = async () => {
    ...
    await saveBoundaryPoint(saved); await refreshProjectData(project.id); setPointDraft(null)
    if (point?.id === saved.id) { setPoint(saved); setScreen('pointDetail') } else setScreen('projectDetail')
  }

  const removePoint = async (target: BoundaryPoint) => {
    if (!project || !window.confirm(`「${target.name}」を削除しますか？\n写真も削除されます。`)) return
    await deleteBoundaryPoint(target.id); await refreshProjectData(project.id)
  }
  ...
  const addPhotos = async (files: File[]) => {
    ...
    for (const [index, file] of files.entries()) {
      const nextPhoto: BoundaryPhoto = { id: crypto.randomUUID(), projectId: project.id, boundaryPointId: point.id, category, fileName: file.name, fileType: file.type, fileSize: file.size, blob: file, createdAt: new Date(base + index).toISOString() }
  ...
  const updatePhotoCategory = async (target: BoundaryPhoto, nextCategory: string) => { if (!point) return; await savePhoto({ ...target, category: nextCategory }); await refreshPhotos(point.id) }
  const removePhoto = async (target: BoundaryPhoto) => { if (!point || !window.confirm('この写真を削除しますか？')) return; await deletePhoto(target.id); await refreshPhotos(point.id); setViewerIndex(null) }
  const savePhotoToDevice = (target: BoundaryPhoto) => { const url = URL.createObjectURL(target.blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = target.fileName || 'photo'; anchor.click(); window.setTimeout(() => URL.revokeObjectURL(url), 0) }
  ...
  const Header = ({ title, onBack }: { title: string; onBack?: () => void }) => <header className="app-header">{onBack && <button type="button" className="back-button" onClick={onBack} aria-label="戻る">‹</button>}<h1>{title}</h1></header>

  if (screen === 'projectEdit' && projectDraft) {
    const editing = project?.id === projectDraft.id
    return <div className="app-shell"><Header title={editing ? '案件編集' : '案件登録'} onBack={() => setScreen(editing ? 'projectDetail' : 'projectList')} /><main className="form-screen">
  ...
  if (screen === 'pointEdit' && pointDraft) {
    ...
    return <div className="app-shell"><Header title={editing ? '境界点編集' : '境界点登録'} onBack={() => setScreen(editing ? 'pointDetail' : 'projectDetail')} /><main className="form-screen">
  ...
  if (screen === 'pointDetail' && project && point) return <div className="app-shell"><Header title={point.name} onBack={() => setScreen('projectDetail')} /><main className="content-screen">
    <section className="card"><div className="card-head"><div><h2>{point.markerType || '境界標未設定'}</h2><p>{point.condition || '状態未設定'}</p></div><button type="button" className="small-button" onClick={() => { setPointDraft({ ...point }); setScreen('pointEdit') }}>編集</button></div><p>{point.positionMemo || '位置関係メモなし'}</p><p>{point.memo || 'メモなし'}</p></section>
    <section><div className="section-head"><h2>写真 <small>{photos.length}枚</small></h2><select value={category} onChange={(e) => setCategory(e.target.value)}>{PHOTO_CATEGORIES.map((value) => <option key={value}>{value}</option>)}</select></div><div className="picker-row"><PickerInput label="📷 撮影" className="camera-picker" inputProps={{ accept: 'image/*', capture: 'environment' }} onFiles={addPhotos} /><PickerInput label="🖼 写真から選択" inputProps={{ accept: 'image/*', multiple: true }} onFiles={addPhotos} /></div><div className="photo-grid">{photos.map((photo, index) => <article className="photo-card" key={photo.id}><PhotoThumb photo={photo} onOpen={() => setViewerIndex(index)} /><select value={photo.category || ''} onChange={(e) => void updatePhotoCategory(photo, e.target.value)}><option value="">未分類</option>{PHOTO_CATEGORIES.map((value) => <option key={value}>{value}</option>)}</select><div className="photo-actions"><button type="button" onClick={() => savePhotoToDevice(photo)}>端末保存</button><button type="button" className="danger-button" onClick={() => void removePhoto(photo)}>削除</button></div></article>)}</div></section>
    {viewerIndex !== null && <PhotoViewer photos={photos} index={viewerIndex} onIndexChange={setViewerIndex} onClose={() => setViewerIndex(null)} />}
  </main></div>

  if (screen === 'projectDetail' && project) return <div className="app-shell"><Header title={project.title} onBack={() => { setProject(null); setPoint(null); setScreen('projectList'); void refreshProjects() }} /><main className="content-screen">
    <section className="card"><div className="card-head"><div><h2>{project.location || '所在地未入力'}</h2><p>{project.lotNumber || '地番未入力'} ／ {project.surveyDate}</p></div><button type="button" className="small-button" onClick={() => { setProjectDraft({ ...project }); setScreen('projectEdit') }}>編集</button></div><p>{project.landCategory || '地目未入力'} ／ {project.boundaryChecked ? '境界確認済み' : '境界未確認'}</p><p>{project.memo || 'メモなし'}</p></section>
  ...
  return <div className="app-shell"><Header title="現場調査" /><main className="content-screen">
    {projects.length === 0 ? <div className="empty-card large">案件がありません<br /><small>右下の＋から案件を登録してください</small></div> : projects.map((item) => {
      ...
      return <div className="project-card" key={item.id}><button type="button" className="project-main" onClick={() => void openProject(item)}><div className="project-title-row"><strong>{item.title}</strong>{percentage !== undefined && <b className={percentage === 100 ? 'project-progress complete' : 'project-progress'}>{percentage === 100 ? '完了' : `${percentage}%`}</b>}</div><span>{item.location || '所在地未入力'} ／ {item.surveyDate}</span></button><button type="button" className="project-delete" onClick={() => void removeProject(item)}>削除</button></div>
    })}
