...
export default function App() {
  ...
  const closeTagDetails = (button: HTMLButtonElement) => {
    const details = button.closest('details')
    window.requestAnimationFrame(() => details?.removeAttribute('open'))
  }

  const closePhotoTagEditor = () => {
    window.requestAnimationFrame(() => setEditingPhotoId(null))
  }
  ...
  const addPhotos = async (files: File[]) => {
    ...
    const base = Date.now()
    const failed: string[] = []
    let lastSavedPhotoId: string | null = null
    ...
    if (lastSavedPhotoId) {
      const targetId = `photo-${lastSavedPhotoId}`
  ...
  const removePhoto = async (target: BoundaryPhoto) => {
    ...
    await refreshPhotos(point.id)
    setEditingPhotoId((current) => current === target.id ? null : current)
    setViewerIndex(null)
  }
  ...
  if (screen === 'pointDetail' && project && point) {
    ...
    return <div className="app-shell point-detail-shell"><Header title={point.name} onBack={() => setScreen('projectDetail')} /><main className="content-screen point-detail-content"><section className="card"><div className="card-head"><div><h2>{point.markerType || '境界標未設定'}</h2><p>{point.condition || '状態未設定'}</p></div><button type="button" className="small-button" onClick={() => { setPointDraft({ ...point }); setScreen('pointEdit') }}>編集</button></div><p>{point.positionMemo || '位置関係メモなし'}</p><p>{point.memo || 'メモなし'}</p></section><section><div className="section-head"><h2>写真 <small>{photos.length}枚</small></h2></div><div className="next-photo-card"><strong>次に撮る写真</strong><label className="next-photo-category"><span>種別</span><select value={category} onChange={(e) => setCategory(e.target.value)}>{PHOTO_CATEGORIES.map((value) => <option key={value}>{value}</option>)}</select></label><div className="point-photo-picker-row"><PickerInput label="📷 撮影" className="point-inline-camera" inputProps={{ accept: 'image/*', capture: 'environment' }} onFiles={addPhotos} /><PickerInput label="🖼 端末から追加" className="point-inline-library" inputProps={{ accept: 'image/*', multiple: true }} onFiles={addPhotos} /></div></div><div className="photo-grid">{photos.map((photo, index) => {
      const isEditingTags = editingPhotoId === photo.id
      return <article className={isEditingTags ? 'photo-card editing' : 'photo-card'} id={`photo-${photo.id}`} key={photo.id}><PhotoThumb photo={photo} onOpen={() => setViewerIndex(index)} /><select value={photo.category || ''} onChange={(e) => void updatePhotoCategory(photo, e.target.value)}><option value="">未分類</option>{PHOTO_CATEGORIES.map((value) => <option key={value}>{value}</option>)}</select><div className="photo-tags photo-tags-summary"><div className="tag-chip-list compact">{(photo.tags ?? []).length === 0 ? <span className="tag-empty">タグなし</span> : (photo.tags ?? []).map((tag) => <span key={tag} className="tag-chip photo-tag-display">{tag}</span>)}</div><button type="button" className="photo-tag-edit-button" onClick={() => setEditingPhotoId(photo.id)}>タグ編集</button>{isEditingTags && <div className="photo-tag-editor-panel"><div className="tag-editor-head"><div><strong>写真のタグ</strong><small>候補を押すとその場で追加・解除します。</small></div><button type="button" className="tag-editor-close" aria-label="タグ編集を閉じる" onClick={(e) => { e.preventDefault(); e.stopPropagation(); closePhotoTagEditor() }}>×</button></div><div className="tag-editor-photo-preview"><div><PhotoThumb photo={photo} onOpen={() => undefined} /></div><strong>この写真を編集中</strong></div><div className="capture-tag-selected-preview"><span>現在</span><div className="tag-chip-list compact">{(photo.tags ?? []).length === 0 ? <span className="tag-empty">タグなし</span> : (photo.tags ?? []).map((tag) => <span key={tag} className="tag-chip selected-preview">✓ {tag}</span>)}</div></div><div className="tag-chip-list capture-tag-choices">{tagChoices.map((tag) => <button key={tag} type="button" className={(photo.tags ?? []).includes(tag) ? 'tag-chip active' : 'tag-chip'} onClick={() => void togglePhotoTag(photo, tag)}>{(photo.tags ?? []).includes(tag) ? `✓ ${tag}` : tag}</button>)}</div><form className="photo-tag-form" onSubmit={(e) => { e.preventDefault(); const input = e.currentTarget.elements.namedItem('tag'); if (!(input instanceof HTMLInputElement)) return; const value = input.value; input.value = ''; void addTagToPhoto(photo, value) }}><input name="tag" placeholder="新しいタグを追加" aria-label={`${photo.fileName}にタグを追加`} /><button type="submit">追加</button></form><button type="button" className="tag-editor-done" onClick={(e) => { e.preventDefault(); e.stopPropagation(); closePhotoTagEditor() }}>完了して閉じる</button></div>}</div><div className="photo-actions"><button type="button" onClick={() => savePhotoToDevice(photo)}>端末保存</button><button type="button" className="danger-button" onClick={() => void removePhoto(photo)}>削除</button></div></article>
    })}</div></section><footer className="point-action-footer" aria-label="境界点の撮影操作"><div className="capture-tag-summary-row"><div className="capture-tag-current" aria-label="現在の撮影タグ"><span className="capture-tag-label">🏷</span><div className="capture-tag-current-list">{captureTags.length === 0 ? <span className="capture-tag-none">タグなし</span> : captureTags.slice(0, 3).map((tag) => <span key={tag} className="capture-tag-current-chip">{tag}</span>)}{captureTags.length > 3 && <span className="capture-tag-more">+{captureTags.length - 3}</span>}</div></div><details className="capture-tag-menu"><summary>変更</summary><div className="capture-tag-menu-panel"><div className="tag-editor-head"><div className="capture-tag-menu-head"><strong>撮影タグを選択</strong><small>選択しても候補の位置は変わりません。</small></div><button type="button" className="tag-editor-close" aria-label="撮影タグ選択を閉じる" onClick={(e) => { e.preventDefault(); e.stopPropagation(); closeTagDetails(e.currentTarget) }}>×</button></div><div className="capture-tag-selected-preview"><span>選択中</span><div className="tag-chip-list compact">{captureTags.length === 0 ? <span className="tag-empty">タグなし</span> : captureTags.map((tag) => <span key={tag} className="tag-chip selected-preview">✓ {tag}</span>)}</div></div><div className="tag-chip-list capture-tag-choices">{tagChoices.map((tag) => <button key={tag} type="button" className={captureTags.includes(tag) ? 'tag-chip active' : 'tag-chip'} onClick={() => toggleCaptureTag(tag)}>{captureTags.includes(tag) ? `✓ ${tag}` : tag}</button>)}</div><div className="tag-add-row"><input value={newCaptureTag} onChange={(e) => setNewCaptureTag(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addCaptureTag() } }} placeholder="新しいタグを追加" aria-label="撮影時タグを追加" /><button type="button" onClick={addCaptureTag}>追加</button></div><button type="button" className="tag-editor-done" onClick={(e) => { e.preventDefault(); e.stopPropagation(); closeTagDetails(e.currentTarget) }}>完了して閉じる</button></div></details></div><div className="point-action-row"><button type="button" className="point-nav-button" disabled={!previousPoint} onClick={() => { if (previousPoint) void openPoint(previousPoint) }}>← {previousPoint?.name ?? '前なし'}</button><div className="point-position" aria-label={`境界点 ${currentPointIndex + 1} / ${points.length}`}>{currentPointIndex + 1} / {points.length}</div><button type="button" className="point-nav-button" disabled={!nextPoint} onClick={() => { if (nextPoint) void openPoint(nextPoint) }}>{nextPoint?.name ?? '次なし'} →</button><PickerInput label="📷" className="camera-picker point-footer-camera" inputProps={{ accept: 'image/*', capture: 'environment' }} onFiles={addPhotos} /></div></footer>{viewerIndex !== null && <PhotoViewer photos={photos} index={viewerIndex} onIndexChange={setViewerIndex} onClose={() => setViewerIndex(null)} />}</main></div>
  }
